...
1. [`01_build_dataset_cache.ipynb`](01_build_dataset_cache.ipynb) audits queue
   reconstruction, creates inferred rank-reach labels for every observed rank,
   validates them, and creates the deterministic per-session cache.
...
6. [`06_seasonal_models_and_export.ipynb`](06_seasonal_models_and_export.ipynb)
   ...
   sets, verifies exact serialized-tree predictions and rank monotonicity, and
   is the only notebook that exports the deployed model bundle and its supported
   rank range.
7. [`07_production_benchmark.ipynb`](07_production_benchmark.ipynb) produces the
   final apples-to-apples baseline results, reports the rank-above-100 slice,
   and creates the figures used in the project README.
