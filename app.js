...
function renderEstimate(code, lecture, position, data) {
  ...
  const historyPane = data.historyStatus === "available"
    ? `<p class="result-summary">A ${safeCode} waitlist recorded at least <strong>${position} ${position === 1 ? "position" : "positions"}</strong> of downward movement in <strong>${data.cleared} of ${data.outcomes.length}</strong> previous offerings where the waitlist was at least ${position} large and there ${data.current.daysRemaining === 1 ? "was" : "were"} ${data.current.daysRemaining} ${data.current.daysRemaining === 1 ? "day" : "days"} before the ${term} waitlist deadline.</p>
