...
const deadlineKey = (code, session) => {
  if (isSummer(session)) return "summerWaitlistClosed";
  return getTerm(code) === "S" ? "winterWaitlistClosed" : "fallWaitlistClosed";
};
...
function capacityAt(meeting, timestamp) {
  ...
  for (const change of complex.capChanges ?? []) {
    const time = change.timing ?? change.timestamp ?? 0;
    if (time <= timestamp) capacity = change.capAfter ?? change.newCap ?? change.capacity ?? capacity;
  }
  return capacity;
}
...
function analyze(course, deadline, daysRemaining, position) {
  const meeting = course.meetings.find(item => !item.isCancelled && item.enrollmentLogs?.length);
  ...
  return { movement, cleared: movement >= position, startWaitlist, instructor: meeting.instructors?.map(i => `${i.firstName} ${i.lastName}`).join(", ") || "Instructor unavailable" };
}
...
async function deadlineFor(session, code) {
  ...
  return faculty?.importantTimestamps?.[deadlineKey(code, session)] ?? null;
}
...
function renderEstimate(code, position, data) {
...
form.addEventListener("submit", async event => {
  ...
  try { renderEstimate(code, position, await estimate(code, position)); }
